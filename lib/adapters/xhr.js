...
module.exports = function xhrAdapter(config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    var requestData = getOwn(config, 'data');
    var requestHeaders = getOwn(config, 'headers');
    var responseType = config.responseType;
    var withXSRFToken = getOwn(config, 'withXSRFToken');
    var onCanceled;
    function done() {
    ...
    if (configAuth) {
      ...
      var rawPassword = getOwn(configAuth, 'password');
    ...
    if (utils.isStandardBrowserEnv()) {
      ...
      if (withXSRFToken === true || (withXSRFToken !== false && isURLSameOrigin(fullPath))) {
        // Add xsrf header
        var xsrfHeaderName = getOwn(config, 'xsrfHeaderName');
        var xsrfCookieName = getOwn(config, 'xsrfCookieName');
